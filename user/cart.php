...
<head>
  ...
  <style>
    ...
    .quantity-control {
      ...
      /* Border tipis di sekitar kontrol */
      border-bottom: 2px solid #ccc;
      /* Border tipis di sekitar kontrol */
      overflow: hidden;
      /* Memastikan tidak ada yang tumpah keluar dari border */
      width: fit-content;
      /* Sesuaikan lebar dengan konten */
      font-family: Arial, sans-serif;
      /* Font yang umum digunakan */
      font-size: 16px;
      /* Ukuran font standar */
    }
    ...
    .quantity-control .buttonq {
      ...
      cursor: pointer;
      /* Kursor berubah menjadi pointer saat di-hover */
      font-weight: bold;
    ...
    .quantity-control .buttonq:hover {
    ...
    .quantity-control input[type="number"] {
      border: none;
      /* Tidak ada border pada input */
      text-align: center;
      /* Teks di tengah input */
      padding: 8px 0;
      /* Padding vertikal untuk input */
      width: 50px;
      /* Lebar input */
      -moz-appearance: textfield;
      /* Untuk Firefox, menyembunyikan panah default */
    }
...
<body>
  ...
  if (isset($_POST['save'])) {
    ...
    for($i = 0; $i < count($checkbox); $i++){
        $check_id = $checkbox[$i];
        $jml = $jmll[$i];
        $id_product = $id_productd[$i];
        $admin_id = $admin_idd[$i];
        mysqli_query($conn, "INSERT INTO t_checkout_temp VALUES ($check_id, $id_product, $jml, $admin_id)");
        mysqli_query($conn, "DELETE FROM t_cart WHERE id_cart=$check_id");
    }
    echo "<script>
